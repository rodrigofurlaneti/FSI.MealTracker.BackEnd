...
namespace FSI.MealTracker.Infrastructure.Repositories
{
    ...
    {
        private const string PROCEDURE_NAME = "usp_Meal";
        ...
        public MealRepository(IDbContext context) : base(context) { }

        public async Task<IEnumerable<MealEntity>> GetAllAsync()
        {
            using var connection = CreateConnection();
            return await connection.QueryAsync<MealEntity>(
                PROCEDURE_NAME,
        ...
        {
            using var connection = CreateConnection();
            return await connection.QueryFirstOrDefaultAsync<MealEntity>(
                PROCEDURE_NAME,
        ...
        public async Task<long> AddAsync(MealEntity entity)
        {
            using var connection = CreateConnection();
        ...
        public async Task<bool> UpdateAsync(MealEntity entity)
        {
            using var connection = CreateConnection();
        ...
        public async Task<bool> DeleteAsync(MealEntity entity)
        {
            using var connection = CreateConnection();
            return await connection.ExecuteScalarAsync<bool>(
                PROCEDURE_NAME,
        ...
        public async Task<IEnumerable<MealEntity>> GetAllFilteredAsync(string filterBy, string value)
        {
            using var connection = CreateConnection();
            if (!_orderMap.ContainsKey(filterBy))
                throw new ArgumentException("Invalid filterBy field");

            var procedureName = $"usp_Meal_GetAll_FilterBy_{_orderMap[filterBy]}";
            var parameters = new DynamicParameters();
            parameters.Add(_orderMap[filterBy], value);

            return await connection.QueryAsync<MealEntity>(procedureName, parameters, commandType: CommandType.StoredProcedure);
        }

        public async Task<IEnumerable<MealEntity>> GetAllOrderedAsync(string orderBy, string direction)
        {
            using var connection = CreateConnection();
            ...
            var procedureName = $"usp_Meal_GetAll_OrderBy_{_orderMap[orderBy]}_{(isDesc ? "Desc" : "Asc")}";

            return await connection.QueryAsync<MealEntity>(procedureName, commandType: CommandType.StoredProcedure);
        }
