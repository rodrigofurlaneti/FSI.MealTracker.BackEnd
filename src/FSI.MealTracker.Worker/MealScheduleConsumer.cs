...
{
    public class MealScheduleConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public MealScheduleConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "meal-schedule-queue";
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<MealScheduleMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IMealScheduleAppService>();
                    ...
                    IEnumerable<MealScheduleDto> listMealSchedule = null;

                    MealScheduleDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "insert":
                            ...
                            break;
                        case "getall":
                            listMealSchedule = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listMealSchedule, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, MealScheduleMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<MealScheduleDto> listMealSchedule, MealScheduleDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listMealSchedule);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, MealScheduleMessage envelope,
            string queueName, long? createdId)
        {
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    MessageResponse = string.Empty,
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert meal schedule into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
                });
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, MealScheduleMessage envelope,
            string queueName, IEnumerable<MealScheduleDto> listMealSchedule)
        {
            if (listMealSchedule != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listMealSchedule);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listMealSchedule),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert meal schedule into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, MealScheduleMessage envelope,
            string queueName, MealScheduleDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, MealScheduleMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, MealScheduleMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
