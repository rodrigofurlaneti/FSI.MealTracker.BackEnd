...
{
    public class DailyGoalConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public DailyGoalConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "daily-goal-queue";

            var factory = new ConnectionFactory
            {
                HostName = _config["RabbitMq:Host"] ?? "localhost",
                UserName = _config["RabbitMq:UserGoal"] ?? "guest",
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<DailyGoalMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IDailyGoalAppService>();
                    ...
                    IEnumerable<DailyGoalDto> listDailyGoal = null;

                    DailyGoalDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "insert":
                            ...
                            break;
                        case "getall":
                            listDailyGoal = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listDailyGoal, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, DailyGoalMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<DailyGoalDto> listDailyGoal, DailyGoalDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listDailyGoal);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, DailyGoalMessage envelope,
            string queueName, long? createdId)
        {
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    MessageResponse = string.Empty,
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert daily goal into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
                });
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, DailyGoalMessage envelope,
            string queueName, IEnumerable<DailyGoalDto> listDailyGoal)
        {
            if (listDailyGoal != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listDailyGoal);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listDailyGoal),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert daily goal into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, DailyGoalMessage envelope,
            string queueName, DailyGoalDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, DailyGoalMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, DailyGoalMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
