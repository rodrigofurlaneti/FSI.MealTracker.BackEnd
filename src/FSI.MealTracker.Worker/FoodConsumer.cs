...
{
    public class FoodConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public FoodConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<FoodMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IFoodAppService>();
                    ...
                    IEnumerable<FoodDto> listFood = null;

                    FoodDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "insert":
                            ...
                            break;
                        case "getall":
                            listFood = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listFood, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, FoodMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<FoodDto> listFood, FoodDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listFood);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, FoodMessage envelope,
            string queueName, long? createdId)
        {
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    MessageResponse = string.Empty,
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert food into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
                });
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, FoodMessage envelope,
            string queueName, IEnumerable<FoodDto> listFood)
        {
            if (listFood != null)
            {
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listFood),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert food into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, FoodMessage envelope,
            string queueName, FoodDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, FoodMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, FoodMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
